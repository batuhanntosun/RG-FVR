...
import torch
import math
# Imports related to  CLIP Image Encoder is now disabled ! Figure out how to use them via Wan 2.1 I2V Pipeline !
...
"""
    Initially we need a face alignment model and ArcFace model (CLIP will be integrated later !)
        - FaceAlignment Model
        - ArcFace Model
        - CLIP Model (later)
        - FaRL Model (later)
...
def prepare_face_models(model_path, device, dtype):
    ...
    # PART 1 is completely disabled due to CLIP Image Encoder !
...
def process_face_embeddings(
    ...
    face_helper_1.get_face_landmarks_5(only_center_face=True) # 5 landmark points can be utilized later !
    if face_kps is None:
        face_kps = face_helper_1.all_landmarks_5[0]
    face_helper_1.align_warp_face() # We also warp the image be careful
...
def process_face_embeddings_infer(
    ...
    id_cond, align_crop_face_image, face_kps = process_face_embeddings(
        face_helper_1,
        # clip_vision_model,
        face_helper_2,
        # eva_transform_mean,
