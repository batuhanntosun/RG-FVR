...
from pipeline_dmd import RGFVRDMDPipeline
...
#   <ckpts_dir>/transformer/           (teacher restorer)
#   <ckpts_dir>/dmd/                   (LoRA weights)
#   <ckpts_dir>/vae/Wan2.1_VAE.pth
#   <ckpts_dir>/text_encoder/
#   <ckpts_dir>/tokenizer/
#   <ckpts_dir>/face_encoder/
#   <ckpts_dir>/farl/
#   <ckpts_dir>/arcface/
...
# Inference Settings
NUM_INFERENCE_STEPS = 3   # DMD: 3 denoising steps
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "RGFVR-DMD: Reference-Guided Face Video Restoration (3-step DMD). "
    ...
    parser.add_argument("--ckpts_dir", type=str, default=DEFAULT_CKPTS_DIR,
                        help=(
                            ...
                            "Expected subfolders: transformer/, dmd/, vae/, text_encoder/, tokenizer/, "
...
def resolve_ckpt_paths(ckpts_dir):
    ...
    return {
        ...
        "model_root": ckpts_dir,
        "transformer": os.path.join(ckpts_dir, "transformer"),
        "dmd_lora": os.path.join(ckpts_dir, "dmd", "generator_lora.safetensors"),
        "face_encoder_root": ckpts_dir,
...
def _find_lora_module(model, lora_key_path):
    """Navigate model hierarchy to find the target Linear module.

    LoRA keys use underscore separators: blocks_0_self_attn_q
    Module paths use dots and may contain underscores: blocks.0.self_attn.q
    We try progressively joining segments with dots vs underscores (greedy match).
    """
    parts = lora_key_path.split("_")
    current = model
    i = 0
    while i < len(parts):
        found = False
...
def merge_lora_into_model(model, lora_path, multiplier=1.0):
    """Merge LoRA weights directly into model's base weights (in-place)."""
    ...
    for key in state_dict:
        ...
        alpha = state_dict[alpha_key].item() if alpha_key in state_dict else weight_down.shape[0]
        scale = alpha / weight_down.shape[0]

        # lora_unet__blocks_0_cross_attn_k.lora_down.weight -> blocks_0_cross_attn_k
        module_path = key.split(".lora_down.weight")[0]
        module_path = module_path.replace("lora_unet_", "")
        module_path = module_path.lstrip("_")
        target = _find_lora_module(model, module_path)

        if target is not None:
            delta = multiplier * scale * torch.mm(
                weight_up.to(target.weight.dtype),
                weight_down.to(target.weight.dtype),
            )
            target.weight.data += delta.to(target.weight.device)
            merged += 1

    print(f"  LoRA merged: {merged} layers")
    if merged == 0:
        print("  WARNING: No LoRA layers merged. Check key format.")
...
def initialize_pipeline(paths):
    print("Initializing RGFVRDMDPipeline...")
    ...
    # Merge DMD LoRA into the loaded teacher weights
    print(f"Merging DMD LoRA from {paths['dmd_lora']} ...")
    merge_lora_into_model(transformer3d, paths["dmd_lora"])
...
def process_single_video(video_path, save_dir, clip_name, pipeline, prompt, ref_path,
                         save_frames=False, fps_override=None):
    gc.collect()
...
def main():
    ...
    if not os.path.isfile(paths["config"]):
        ...
        return
    if not os.path.isfile(paths["dmd_lora"]):
        print(f"Error: DMD LoRA weights not found at {paths['dmd_lora']}")
        return
    ...
    print(f"\n{'='*60}")
    print(f"Single Clip DMD Inference: {clip_name}")
    print(f"Ckpts dir: {paths['ckpts_dir']}")
    print(f"DMD LoRA: {paths['dmd_lora']}")
    print(f"Settings: Steps={NUM_INFERENCE_STEPS} | Frames={N_FRAMES if N_FRAMES else 'All'} | "
