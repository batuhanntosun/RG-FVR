...
class FaRLFaceAttribute(FaceAttribute):
    ...
    def forward(self, images: torch.Tensor, data: Dict[str, Any], warp_opt: bool = True, transforms= None):
        setting = pretrain_settings[self.conf_name]
        ...
        if warp_opt:
            ...
            w_images = F.grid_sample(simages, 
                                    grid, 
                                    mode="bilinear", 
                                    align_corners=False)
        else:
            if transforms is not None:
                w_images = transforms(simages)
            else:
